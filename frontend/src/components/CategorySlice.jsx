...
const CategorySlice = ({ title, category }) => {
        ...
        <div className='my-5'>
            ...
            <div className='relative'>
                <div className='flex gap-5 overflow-auto scrollbar-none py-3 transition-all duration-700' ref={scrollElment}>
                    {products && products?.slice(0, 10).map((product, i) => (
                        <div key={i} className='rounded-md flex items-center w-full h-full shadow-md'>
                            <Link to={`/product/${product._id}`} className='bg-gray-200 w-[150px] h-[150px] rounded-l-md p-3 group cursor-pointer'>
                                <img src={product.images[0]} alt="" className='w-full h-full object-contain mix-blend-multiply group-hover:scale-105 transition duration-300' />
                            </Link>
            ...
            {isLoading && <div className='flex items-center gap-5 overflow-hidden'>
                {[...Array(10)].map((array, i) => (
                    <div className="animate-pulse flex" key={i}>
                        <div className="bg-slate-200 w-[150px] h-[150px] mb-2"></div>
                        <div className="bg-white w-[170px] h-[150px] mb-2 py-5 px-3 flex flex-col justify-between gap-2">
