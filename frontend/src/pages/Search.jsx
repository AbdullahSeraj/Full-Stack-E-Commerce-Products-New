...
import { useSearchProductsQuery } from '../redux/features/product/productApiSlice';
import ProductsVertical from '../components/ProductsVertical';
import notFound from "../../public/assest/notFound.png"

const Search = () => {
    const { search } = useLocation();
    const { data, isLoading, isError, error } = useSearchProductsQuery(search)

    return (
        <div className='px-10 py-6'>
            {(!isLoading && !isError) &&
                <div>
                    <h2 className='text-2xl font-semibold'>Search Results: {data?.length}</h2>
                    <ProductsVertical data={data} isLoading={isLoading} />
                </div>
            }
